...
lbl_space8 = Label(window, text="")
lbl_space8.pack()
lbl_title = Label(window, text="Connect by entering 'log *username* *password*'", font = 8)
lbl_title.pack()
lbl_space7 = Label(window, text="")
lbl_space7.pack()
...
msg_list = Listbox(messages_frame, height=15, width=75, yscrollcommand=scrollbar.set, xscrollcommand=True, selectmode = SINGLE, fg="black", activestyle  = "none", selectforeground = "black", selectbackground  = "white", selectborderwidth  = 0)
...
def receive():
    while True:        
        ...
        try:
            ...
            elif ((not loged) and (message[0:23] == "Bienvenue sur le chat  ")):
                #Just save the infos don't write
                infos = message.split('  ')
                login = infos[1]
                ip = infos[5]
                date_connection_str = "Connection date\n" + infos[3]
                loged = True
                
                login_client.set(login)
                ip_client.set(ip)
                date_connection_client.set(date_connection_str)
                lbl_title["text"]="Welcome " + login
            else:
                msg_list.insert(END, message)
